...
plot_list = [
    ("log_weight", r"$\log_{10}$Weight", "np.log10(weight)", -3, 3, 0.1),
]
blind_match = {}
        ...
        for proc in proc_match:
            ...
            for ym in year_match[year]:
                ...
                for pm in natsort.natsorted(os.listdir(dirname)) if proc == "Data" else proc_match[proc]:
                    ...
                    for cut in proc_cut.get("all", []) + proc_cut.get(proc, []):
                    ...
                    for i, (n, l, ex, b, e, s) in enumerate(plot_list):
                        bev = events
                        bevex = np.log10(np.maximum(events["weight"], 1e-20))
                        if proc == "Data":
                            for bl, br in blind_match.get(n, []):
                                mask = (bevex < bl) | (bevex > br)
                                bev = bev[mask]
                                bevex = bevex[mask]
                        hist, _ = np.histogram(bevex.to_numpy(), bins=np.arange(b, e + s, s))
...
for year in list(year_match):
    for mode in modes:
        for i, (n, l, ex, b, e, s) in enumerate(plot_list):
            ...
            hep.histplot(hs, density=True, histtype="step", label=ls, color=cs)
            cms_label(year)
