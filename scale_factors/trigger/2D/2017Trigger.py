...
ax = plt.gca()
try:
    hep.cms.label(data=False, paper=False, supplementary=False, year=2017, ax=ax, fontname="sans-serif", loc=1)
except Exception:
...
plt.savefig("2017QCDTriggerSF.pdf", bbox_inches="tight")
plt.show()
...
def SF2DUnc_withvalue(eventsMC, eventsData, x_label, y_label, isData=True, xmin=200, xmax=1200, bins=500, legend_location="best"):
    ...
    ax = plt.gca()
    try:
        hep.cms.label(data=isData, paper=False, supplementary=False, year=2017, ax=ax, fontname="sans-serif")
    except Exception:
    ...
    mask = np.zeros(len(eventsMC), dtype='bool')
    for Trigger in TriggerList: mask |= (eventsMC[Trigger] == 1).to_numpy().astype('bool')
    eventsMC_Cut = eventsMC[mask]
    ...
    mask = np.zeros(len(eventsData), dtype='bool')
    for Trigger in TriggerList: mask |= (eventsData[Trigger] == 1).to_numpy().astype('bool')
    eventsData_Cut = eventsData[mask]
    ...
    plt.savefig("2017Unc" + y_label + "_vs_" + x_label + "TriggerEffiSF.pdf", bbox_inches="tight")
...
def plot_effi_withvalue(eventsMC, eventsData, x_label, y_label, isData=True, xmin=200, xmax=1200, bins=500, legend_location="best"):
    ...
    plt.xlabel(r"Wcb candidate jet $m_{SD}$", fontsize=32, ha="right", x=1)
    plt.ylabel(r"Wcb candidate jet $p_{T}$", fontsize=32, ha="right", y=1)
    plt.savefig("2017WithValue" + y_label + "_vs_" + x_label + "TriggerEffiSF.pdf", bbox_inches="tight")
